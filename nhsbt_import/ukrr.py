...
BASE_PATH = "Q:\\SVNData\\2019\\"
QUARTER_FOLDER = "Q100"


def get_xml_value(parentnode, xpath):
...
def process_cracked_file(filepath, demographics_only=True):
    ...
    for IDNNode in root.xpath("IDN"):
        ...
        output_row = [rr_no, surname, forename, sex, dob, local_hosp_no, chi_no, nhs_no, hsc_no]
        ...
        if not demographics_only:
            ...
            output_row.extend([hosp_centre, qua_modality, qua_treat_sup, qbl_modality, qbl_treat_sup, prd_code, ethnicity])
...
def process_guys_csv(filepath, demographics_only=True):
...
def process_bhly_xls(filepath, demographics_only=True):
    ...
    for i in range(0, 4):
        ...
        for row_index in range(1, worksheet.nrows):
            ...
            if not demographics_only:
                ...
                qua_modality = bhly_modality_map.get(worksheet.cell(row_index, 4).value, None)
                ...
                qua_treat_sup = bhly_treat_sup_map.get(worksheet.cell(row_index, 4).value, None)
                qbl_modality = None
                qbl_treat_sup = None
...
def process(demographics_only=True):
            ...
            if os.path.isdir(BASE_PATH + unit_folder):
                ...
                if unit_code in ("RJ121",):
                    ...
                    if os.path.exists(filepath):
                        ...
                        output_list.extend(process_guys_csv(filepath))
                ...
                # BHLY
                    ...
                    if os.path.exists(filepath):
                        file_found = True
                        print("Proccessing", filepath)
                        output_list.extend(process_bhly_xls(filepath, demographics_only))
                ...
                # Normal UKRR Files
                    ...
                    if file_found:
                        print("Proccessing", filepath)
                        output_list.extend(process_cracked_file(filepath, demographics_only))
                        continue
